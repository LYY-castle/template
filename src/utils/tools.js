...
// 上传功能 FILE_SERVER_URL=服务器地址 url=上传地址 ftomData=上传的流 date=日期 YYYY-MM-DD timeStamp=时间戳
export function uploading(FILE_SERVER_URL, url, formData, date, timeStamp, name) {
  let DLurl = []
  ...
  xhr.onreadystatechange = () => {
    if (xhr.status === 200 && xhr.readyState === 4) {
      ...
      if (formData.file) {
        DLurl.push({ 'name': formData.file.name, 'url': `${FILE_SERVER_URL}/crm/${localStorage.getItem('agentId') + '/' + date + '/' + timeStamp + '_' + formData.file.name}` })
        sessionStorage.setItem('DLurl', JSON.stringify(DLurl))
      } else {
        DLurl.push({ 'name': name, 'url': `${FILE_SERVER_URL}/crm/${localStorage.getItem('agentId') + '/' + date + '/' + timeStamp + '_' + name}` })
        sessionStorage.setItem('DLurl', JSON.stringify(DLurl))
      }
    }
  }
