  ...
  <div class="app-container">
    <div class="filter-container">
      <el-row class="form-container" style="padding:20px 0 0 0;margin-bottom:0;">
        ...
        >
          <span
            class="form-more bold font14"
            style="position:absolute;top:15px;right:45px;color:#57AFFF;"
          >更多</span>
            ...
            >
              <el-row style="padding-left:14px">
                ...
                <el-form-item label="客户质量评分:">
                  <el-input
                    v-model="req.customerScoreStart"
                    placeholder="客户质量评分"
                    size="small"
                    style="width:70px;"
                  />
                ...
                <el-row style="display:inline-block;">
                  <el-form-item label="地区:">
                    <el-select
                      v-model="req.customerProvince"
                      @change="findRegionByRegionParentId(req.customerProvince,2)"
                    >
                      <el-option value label="--省份/地区--" />
                      <el-option
                  ...
                  <el-form-item>
                    <el-select
                      v-model="req.customerCity"
                      @change="findRegionByRegionParentId(req.customerCity,3)"
                    >
                      <el-option value label="--城市--" />
                      <el-option
        ...
        <el-row style="margin-top:20px;">
          ...
          >
            ...
            <el-table-column
              ...
              :show-overflow-tooltip="true"
            >
              <template slot-scope="scope">{{ scope.row.listName }}</template>
            </el-table-column>
            <el-table-column align="center" prop="totalNum" label="总数量" />
            <el-table-column align="center" prop="noUseNum" label="可分配数量" />
            <el-table-column
              align="center"
              prop="modifierName"
              label="操作人"
            ...
            <el-table-column align="center" prop="modifierTime" label="操作时间" width="155" />
          </el-table>
          <el-row style="margin-top:1%;">
            <el-col :span="12">
              <div style="margin-top:1%;" class="font14">
                已选中名单统计：总数量
                <span style="color:red">{{ formInline.totalNum }}</span>，可分配数量
                <span style="color:red">{{ formInline.noUseNum }}</span>
              </div>
            </el-col>
            <el-col :span="12">
              <el-pagination
      ...
      <el-row class="table-container" style="margin-top:15px">
        ...
        <el-row style="margin-bottom:15px;">
          <div>
            <span class="font14" style="color:red;">提示：可在下方表格中最后一列手动填写分配数量或由系统自动完成分配动作。</span>
          </div>
        </el-row>
        <el-row>
        ...
        <el-row style="margin-top:20px;">
          ...
          >
            ...
            <el-table-column align="center" prop="noUseNum" :label="noUseNumLabel" />
            <el-table-column align="center" prop="useNum" :label="useNumLabel" />
            <el-table-column align="center" prop="totalNum" label="总数量" />
            <el-table-column align="center" label="本次分配数量">
              <template slot-scope="scope" class="padding-right-0">
                <el-input
                  v-model="tableData2[scope.$index].num"
                  @change="handleChange1(scope.row)"
                />
              </template>
            </el-table-column>
          </el-table>
...
import {
  ...
  assignTaskInfo,
  findAllBatch
} from '@/api/captain_list_newdistribute_group'
import { Message } from 'element-ui'
import {
  findCampaignByUser,
  getDownInfoByCurrentUser
} from '@/api/monitor_list_single'
import { formatDateTime, findRegionByRegionParentId } from '@/utils/tools'

export default {
  name: 'CaptainListNewdistributeGroup',
  data() {
    return {
      formContainerOpen: [],
      loading: false,
  ...
  methods: {
      ...
      findCampaignNameListAssignInfo(this.req).then(response => {
        if (response.data.code === 0) {
          ...
          for (let i = 0; i <= this.tableData.length - 1; i++) {
            if (this.tableData[i]) {
              this.tableData[i].modifierTime = formatDateTime(
                this.tableData[i].modifierTime
              )
            }
          }
          findDownNamelistInfoByCampaignAndDepart({
              ...
              for (let i = 0; i <= this.tableData2.length - 1; i++) {
                if (this.tableData2[i]) {
                  ...
                  if (this.tableData2[i].staffName) {
                    this.tableData2[i].departName = this.tableData2[
                      i
                    ].staffName
                  }
                }
    ...
    assignNum() {
      ...
      if (this.multipleSelection2.length !== 0) {
        for (let i = 0; i <= this.multipleSelection2.length - 1; i++) {
          this.multipleSelection2[i].num = parseInt(
            Number(this.formInline.num) / this.multipleSelection2.length
          )
        }
        for (
          let i = 0;
    ...
    confirm() {
      ...
      if (this.multipleSelection.length === 0) {
        ...
        Number(this.formInline.num) > Number(this.formInline.noUseNum)
      ) {
        Message({
          message: '分配量不能大于未分配量',
          type: 'error',
        ...
        for (let i = 0; i <= this.multipleSelection2.length - 1; i++) {
          ...
          if (this.multipleSelection2[i]) {
            if (this.multipleSelection2[i].departId) {
              obj[
                String(this.multipleSelection2[i].departId)
              ] = this.multipleSelection2[i].num
            } else {
              obj[
                String(this.multipleSelection2[i].staffId)
              ] = this.multipleSelection2[i].num
            }
            data.push(obj)
          }
