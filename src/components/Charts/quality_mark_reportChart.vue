<template>
  ...
  <div style="width: 100%;height: 90%" v-else-if="staffPermission">
    ...
    <el-row class="table-container">
      ...
      <el-row @click="page(item,index)" style="margin-top:20px;">
        <el-pagination
  ...
  export default {
      ...
      getDepartId().then(res => {
        ...
        permsManager(localStorage.getItem('accountNo')).then(response => {
          ...
          if (code === 200) {
            ...
            permsmarkreportdepart(localStorage.getItem('accountNo')).then(r => {
              ...
              if (code === 200) {
                ...
                departAgents(res.data.departId).then(response => {
                  ...
                  permsmarkreportstaff(localStorage.getItem('accountNo')).then(re => {
    ...
    methods: {
      ...
      handleCurrentChangeStatics(val) {
        ...
        qualityReportstatistics(params).then(response => {
      ...
      initChart1() {
        ...
        this.chartStaff.setOption({
          backgroundColor: '#344b58',
          title: {
            text: this.statistics_type === 'depart' ? '单个时间各部门质检计分报表' : '单个时间各员工质检计分报表',
            x: '20',
            top: '20',
            textStyle: {
              color: '#fff',
      ...
      initChart2() {
        ...
        this.chartTime.setOption({
          backgroundColor: '#344b58',
          title: {
            text: this.statistics_type === 'depart' ? '单个部门各时间段质检计分报表' : '单个员工各时间段质检计分报表',
            x: '20',
            top: '20',
            textStyle: {
              color: '#fff',
      ...
      search(val) {
        ...
        if (this.timeValue && this.timeValue[0] && this.timeValue[0].getDate() && this.timeValue[1] && this.timeValue[1].getDate() && (this.timeValue[0] > this.timeValue[1])) {
          ...
          qualityReporttotalAgent(params).then(response => {
